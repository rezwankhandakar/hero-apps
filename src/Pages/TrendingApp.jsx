...
const TrendingApp = () => {
    ...
    return (
        <div className='bg-[#D2D2D2] text-center'>
            ...
            {/* App Grid */}
            <div className='h-auto w-10/12 mx-auto px-4 py-3'>
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
                    {apps.map(app => (
                        <Link key={app.id} to={`/appDetails/${app.id}`}> 
                            <div className='bg-white rounded-md shadow-md hover:shadow-lg transition-shadow duration-300'>
                                <img className='w-36 h-36 sm:w-40 sm:h-40 md:w-48 md:h-48 mx-auto pt-4 pb-1 object-contain' src={app.image} alt={app.companyName} />
                                <h2 className='text-black text-[14px] font-semibold text-center mt-2'>{app.companyName}</h2>

                                <div className='flex justify-between items-center px-2 pb-3 pt-2'>
                                    <div className='bg-[#f1f5e8] h-7 w-16 rounded-sm flex justify-between items-center px-1'>
                                        <FontAwesomeIcon icon={faDownload} className="text-[14px] text-[#00D390]" />
                                        <p className='text-[14px] font-bold text-[#00D390]'>{app.downloads}</p>
                                    </div>
            ...
            <NavLink to='/allApplication'>
                <button className='h-8 w-28 rounded-sm bg-gradient-to-r from-[#632EE3] to-[#9F62F2] text-[14px] my-5'>
                    Show All
                </button>
            </NavLink>
        </div>
